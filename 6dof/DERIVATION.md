...
$$
\mathbf{Q} = \text{diag}(\sigma\_{ng}^2 \mathbf{I}\_3, \sigma\_{na}^2 \mathbf{I}\_3, \sigma\_{nbg}^2 \mathbf{I}\_3, \sigma\_{nba}^2 \mathbf{I}\_3)
$$
...
For **SO3** states, the transition uses $\mathbf{A}(-\Delta t \cdot \tilde{\boldsymbol{\omega}})^\top$ to map rotation error. For **S2** (gravity), the transition uses $N\_x \cdot R\_{exp} \cdot M\_x$ between tangent spaces.
...
$$
\mathbf{y}\_{pose} = \begin{bmatrix} \mathbf{z}\_p - \mathbf{h}\_p(\mathbf{x}) \\ \log(R\_{obs} \cdot R\_{pred}^{-1}) \end{bmatrix}
$$
...
\frac{\partial \log(\mathbf{R} \mathbf{R}\_{gnss})}{\partial \boldsymbol{\phi}^{R\_{gnss}}} = \mathbf{J}\_r^{-1}(\boldsymbol{\phi}\_{hat})
$$
...
The **iterated** update repeats with the current $\mathbf{x}$ and $\mathbf{H}$ until $\|\delta\mathbf{x}\_i\| < \epsilon\_i$ (typical $\epsilon \sim 10^{-4}$) or max iterations (5–10).
...
| Rotation | $\log(\mathbf{R}\mathbf{R}\_{gnss})$ | $\mathbf{0}$ | $\mathbf{J}\_r^{-1}[\boldsymbol{\phi}] \mathbf{R}\_{gnss}^\top$ | $\mathbf{0}$ | $\mathbf{0}$ | $\mathbf{0}$ |
